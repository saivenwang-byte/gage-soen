/** 与 frontend/src/data/sceneTaxonomy.js 保持同步 */
export const SCENE_TAXONOMY = {
  all: {
    subcategories: [
      { key: 'all', label: '热门推荐' },
      { key: 'weekend', label: '周末去哪', match: ['周末', '学生', '团建', '海景', 'K歌', '麻将', '羽毛球', '篮球'] },
      { key: 'value', label: '超值捡漏', match: ['折', '免', '半价', '临期', '清仓'] },
    ],
  },
  coffee: {
    subcategories: [
...
export function getSubcategoryConfig(sceneKey, subKey) {
  const subs = SCENE_TAXONOMY[sceneKey]?.subcategories || SCENE_TAXONOMY.all.subcategories;
